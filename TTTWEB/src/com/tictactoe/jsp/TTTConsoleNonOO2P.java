...
public class TTTConsoleNonOO2P {
   ...
   public static int[][] pboard = null;// = new int[ROWS][COLS]; // game board in 2D array for web app
   ...
   //below method is used for web app, console app still remain functional 
   ...
   public void movePlayer(int theSeed, int rw, int cl, int tableSize) {

	         if (theSeed == CROSS) {
	            System.out.print("\nPlayer 'X', enter your move (row[1-3] " + rw + " column[1-3]): " + cl);
	         } else {
	        	System.out.print("\nPlayer 'O', enter your move (row[1-3] " + rw + " column[1-3]): " + cl);
	         }
	         int row = rw - 1;  // array index starts at 0 instead of 1
	         int col = cl - 1;
	         if (row >= 0 && row < tableSize && col >= 0 && col < tableSize && pboard[row][col] == EMPTY) {
	            currntRow = row;
	            currentCol = col;
	            pboard[currntRow][currentCol] = theSeed;  // update game-board content
//	            pboard = board;
	         } else {
	            System.out.println("\nThis move at (" + (row + 1) + "," + (col + 1)
	                  + ") is not valid. Try again...");
	         }
   ...
   public int gameStatus(int theSeed, int currentRow, int currentCol, int tableSize) {
	   int status = PLAYING;   
	   if (isWon(theSeed, currentRow, currentCol, tableSize)) {  // check if winning move
	         status = (theSeed == CROSS) ? CROSS_WON : NOUGHT_WON;
	      } else if (drawChecking(tableSize)) {  // check for draw
	         status = DRAW;
	      }
   ...
   public boolean wonChecking(int theSeed, int currentRow, int currentCol, int tableSize, int mode){
	   ...
	   switch(mode){
		   ...
		   break;
		   case 2 : // n-in-the-column checking
			   for (int i = 0; i < tableSize; ++i) {
		   ...
		   break;
		   case 3 : // n-in-the-diagonal checking
			   for (int i = 0; i < tableSize; ++i) {
		   ...
		   break;
		   case 4 : // n-in-the-opposite-diagonal checking
